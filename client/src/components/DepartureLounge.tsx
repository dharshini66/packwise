...
type Weather = { city: string; temperature: number; condition: string; suggestions: string[] };
...
function flagFor(destination: string) {
  ...
  if (name.includes("london") || name.includes("england") || name.includes("uk")) return "🇬🇧";
  if (name.includes("new york") || name.includes("usa") || name.includes("america")) return "🇺🇸";
  if (name.includes("buenos") || name.includes("argentina")) return "🇦🇷";
  if (name.includes("goa") || name.includes("india")) return "🇮🇳";
  return "🌍";
}

function imageFor(destination: string) {
  const city = destination.toLowerCase();
  if (city.includes("tokyo")) return "https://images.unsplash.com/photo-1540959733332-eab4deabeeaf?auto=format&fit=crop&w=1200&q=85";
  if (city.includes("paris")) return "https://images.unsplash.com/photo-1502602898657-3e91760cbb34?auto=format&fit=crop&w=1200&q=85";
  if (city.includes("london")) return "https://images.unsplash.com/photo-1513635269975-59663e0ac1ad?auto=format&fit=crop&w=1200&q=85";
  if (city.includes("new york")) return "https://images.unsplash.com/photo-1522083165195-3424ed129620?auto=format&fit=crop&w=1200&q=85";
  if (city.includes("beach") || city.includes("goa")) return "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=1200&q=85";
...
function WeatherAdvisory({ journey, weather, error, onAdd }: { journey: Journey; weather: Weather | null; error: string; onAdd: (name: string) => void }) { const suggestions = weather?.suggestions ?? []; return <section className="weather-advisory"><div className="weather-hero"><span className="weather-icon">{weather?.condition === "Rain" ? "🌦️" : weather?.condition === "Cold" ? "❄️" : weather?.condition === "Hot" ? "☀️" : "🌤️"}</span><div><h2>{weather ? `${weather.condition} & ${weather.temperature}°C` : "Retrieving weather clearance…"}</h2><p>{weather ? `${weather.city} · Live local conditions` : "PackWise is contacting the local forecast desk."}</p></div></div>{error && <p className="mt-5 text-[#8b1e3f]">{error}</p>}{weather && <ul className="weather-notes"><li>🌞 Pack according to live {weather.condition.toLowerCase()} conditions in {weather.city}.</li><li>💧 Check your manifest for weather-ready layers and hydration.</li></ul>}<p className="weather-kicker">SUGGESTED PACKING ADDITIONS</p><div className="weather-additions">{suggestions.map((suggestion) => <button onClick={() => onAdd(suggestion)} key={suggestion}><span><b>{suggestion}</b><small>{weather?.condition} weather clearance</small></span><Plus /></button>)}</div></section>; }
